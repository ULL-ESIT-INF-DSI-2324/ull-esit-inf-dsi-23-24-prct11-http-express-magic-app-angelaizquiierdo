...
export const ActualizarCarta = (
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.promises.access(rutaArchivo, fs.constants.F_OK)
      .then(() => {
        const newData = JSON.stringify(nuevaCarta, null, 1);

        fs.promises.readFile(rutaArchivo, 'utf8').then(data => {
...
export const Anadir = (
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.promises.stat(rutaDirectorio)
      .then(stats => {
        // Si el directorio existe, procedemos a añadir la carta
        const newData = JSON.stringify(nuevaCarta, null, 1);

        fs.promises.writeFile(rutaArchivo, newData)
          .then(() => {
            resolve("Carta añadida a la coleccion correctamente miembro del directorio existente");
          })
          .catch(err => {
